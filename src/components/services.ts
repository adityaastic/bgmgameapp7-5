        ...
        return AsyncStorage.getItem('userData').then(async (value: any) => {
            ...
            try {
                ...
                if (error.response) {
                    ...
                    if (error.response.status == 401) {
                        if (error.response.data.message == "Your account has been deactivated please contact administrator") {
                            navigation.reset({
                                index: 0,
                        ...
                        Toast.showWithGravity(error.response.data.message, Toast.LONG, Toast.BOTTOM);                        //  return { status: "false", message: '' };
                    }
                    else if (error.response.status === 0) {
                        return { status: 'noNet', message: 'No internet Connection' };
                    }
