...
const useHarrafGame = (market) => {
  ...
  const setHarrafBets = () => { 
    ...
    andarNumbers?.map(andarNumber => {
      for (andarNumber.number; andarNumber.number < 100; andarNumber.number + 10) {
  ...
  const handleAndarHarrafBets = () => {
    const bets = []
    andarNumbers?.map(andarNumber => {
      for (let betNumber = parseInt(andarNumber.number) * 10 ; betNumber < ((parseInt(andarNumber.number) * 10) + 10) ; betNumber++) {
        bets.push({betKey: betNumber.toString().padStart(2, "0") , points : andarNumber.points/10, betType: "Harraf", harrafType: "Andar", market: market?.market})
      }
    })
    return bets
  }
  ...
  const handleBaharHarrafBets = () => {
    ...
    baharNumbers?.map(baharNumber => {
      for (let betNumber = parseInt(baharNumber.number) ; betNumber < 100; betNumber += 10) {
        bets.push({betKey: betNumber.toString().padStart(2, "0") , points : baharNumber.points/10, betType: "Harraf", harrafType: "Bahar", market: market?.market})
      }
    })
    return bets
  }
  ...
  return { andarNumbers, baharNumbers, setAndarNumber, setBaharNumber, setHarrafBets, allBets, calculateTotalPoints }
}
