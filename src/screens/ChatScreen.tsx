...
const ChatScreen = () => {
  ...
  const { chatScreen, sendMessage, } = useDepositChat({ user: mobile });
  ...
  const onStartRecord = async () => {
    ...
    try {
      ...
      audioRecorderPlayer.addRecordBackListener((e) => {
        ...
        const currentTime = audioRecorderPlayer.mmssss(Math.floor(e.currentPosition));
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#ffffff' }}>
      ...
      <View style={{ flex: 1, padding: 20, justifyContent: 'flex-end' }}>
        <ScrollView showsVerticalScrollIndicator={false} ref={scrollViewRef} style={styles.messagesContainer}>
                  ...
                  <View>
                    <View style={{ flexDirection: 'row', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <TouchableOpacity onPress={() => {
                        ...
                        <FontAwesomeIcon
                          name={currentAudio === imageApiClient + msg.audio && isPlaying ? "pause" : "play"}
                          size={20}
                          color="black"
                        />
                      </TouchableOpacity>
                      <Slider
                        ...
                        minimumValue={0}
                        maximumValue={1}
                        value={isFinite(audioState[imageApiClient + msg.audio]?.durationSeconds) && isFinite(audioState[imageApiClient + msg.audio]?.playSeconds) ? audioState[imageApiClient + msg.audio]?.playSeconds / audioState[imageApiClient + msg.audio]?.durationSeconds : 0}
                        minimumTrackTintColor="#111000"
                        maximumTrackTintColor="#000000"
                        onSlidingComplete={(value) => onSeek(value, imageApiClient + msg.audio)}
                      />
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      <Text>{audioState[imageApiClient + msg.audio]?.playTime || "00:00"}</Text>
                      <Text>{audioState[imageApiClient + msg.audio]?.duration || "00:00"}</Text>
                    </View>
                  </View>
                )}
      ...
      {isRecordingActive && (
        <View style={{ backgroundColor: 'black', width: '35%', padding: 5, borderRadius: 10 }}>
      ...
      <View style={styles.inputContainer}>
        <TextInput
          style={{ flex: 1, borderRadius: 90, padding: 10, borderWidth: 1, borderColor: 'rgba(0, 0, 0, 0.2)' }}
          value={message}
