...
const SpinWheelSecond = () => {
  ...
  const handleSpinClick = () => {
    ...
    if (wheelRef.current && playerInfo && playerInfo?.data?.spin_remaining !== 0) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={styles.textSpin}>
        SPINS LEFT 👉 {playerInfo?.data?.spin_remaining || (!zeroSpin ? "Calculating spins left.." : 0)}
      </Text>
      <View style={{ marginTop: 20 }}>
        <LuckyWheel
          ref={wheelRef}
          slices={isImageMode ? require('./data/slices-for-image.json') : require('./data/slices-for-svg.json')}
